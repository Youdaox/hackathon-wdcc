...
const DEFAULT_STATE: LeaderboardSnapshot = {
  ...
  rules: { dailyBaseEncouragements: 3, encouragementsPerTask: 1, pointsPerTask: 10, pointsPerEncouragementReceived: 4, timezone: "UTC" },
};
...
export class DatabaseLeaderboardRepository implements LeaderboardRepository {
  async read<T>(operation: (snapshot: Readonly<LeaderboardSnapshot>) => T): Promise<T> {
    ...
    const snapshot = (result[0]?.snapshot ?? DEFAULT_STATE) as LeaderboardSnapshot;
