...
export interface Companion {
  ...
  /** Most recently logged meal; together with its timestamp it prevents duplicate daily prompts. */
  lastMeal: Meal | null;
...
export interface FocusSession {
  ...
  xpMultiplier: number;
  /** Check-in modifier active when the session ended. 1 = neutral. */
  emotionalXpMultiplier: number;
  /** Multiplies health lost to distractions; does not affect health regenerated by focus. */
...
export interface ActiveSession {
  ...
  bonusXp: number;
  /** Latest in-session check-in modifiers. They replace rather than stack. */
  emotionalXpMultiplier: number;
  hpLossMultiplier: number;
}
