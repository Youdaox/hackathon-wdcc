...
export async function POST(request: Request) {
  ...
  const [check] = await db.select().from(recallChecks).where(and(eq(recallChecks.id, checkId), eq(recallChecks.userId, user.id)));
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      await tx.update(recallChecks).set({
        status: "submitted", score, xpAwarded, feedbackJson: JSON.stringify(results), submittedAt: Date.now(),
      }).where(and(eq(recallChecks.id, check.id), eq(recallChecks.status, "ready")));
      await tx.update(studyMemorySessions).set({ status: "submitted" }).where(eq(studyMemorySessions.id, check.memorySessionId));
    });
