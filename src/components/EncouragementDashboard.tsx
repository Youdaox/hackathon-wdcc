...
export function EncouragementDashboard() {
  ...
  return <div className="space-y-6">
    ...
    <section id="leaderboards" className="grid gap-6 lg:h-[42rem] lg:grid-cols-2"><div className="card flex min-h-0 flex-col overflow-hidden"><div className="border-b border-line-soft p-6"><p className="eyebrow">Encouragement feed</p><h2 className="mt-1 text-xl font-bold">Your friends are cheering you on</h2><p className="mt-2 text-sm text-muted">New messages appear here automatically.</p></div>{received.length ? <div className="min-h-0 flex-1 divide-y divide-line-soft overflow-y-auto">{received.map((item) => <article key={item.id} className="p-5"><p className="font-bold">{item.senderName}</p><p className="mt-1 text-sm text-muted">“{item.message}”</p></article>)}</div> : <p className="p-8 text-center text-sm text-muted">No encouragements yet — invite a friend to send one.</p>}</div><div className="card flex min-h-0 flex-col overflow-hidden"><div className="flex items-center justify-between border-b border-line-soft p-6"><div><p className="eyebrow">Leaderboard</p><h2 className="mt-1 text-xl font-bold">Community momentum</h2></div><div className="flex rounded-xl bg-surface-2 p-1">{(["week", "month"] as const).map((tab) => <button key={tab} onClick={() => setPeriod(tab)} className={`rounded-lg px-3 py-2 text-xs font-semibold ${period === tab ? "bg-moss text-white" : "text-muted"}`}>{tab}</button>)}</div></div>{boards[period].length ? <div className="min-h-0 flex-1 divide-y divide-line-soft overflow-y-auto">{boards[period].map((entry) => <div key={entry.userId} className="flex items-center justify-between px-5 py-4"><span className="font-semibold">#{entry.rank} {entry.displayName}{entry.userId === currentUser.id ? " (You)" : ""}</span><span className="tabular font-bold text-moss">{entry.score}</span></div>)}</div> : <p className="p-8 text-center text-sm text-muted">Complete a task to join the leaderboard.</p>}</div></section>
    <FocusLeaderboard entries={focusBoard} currentUserId={currentUser.id} />
  </div>;
}
...
function FocusLeaderboard({ entries, currentUserId }: { entries: FocusLeaderboardEntry[]; currentUserId: string }) {
  return <div className="card overflow-hidden lg:col-span-2"><div className="border-b border-line-soft p-6"><p className="eyebrow">Focus leaderboard</p><h2 className="mt-1 text-xl font-bold">Time focused, side by side</h2><p className="mt-2 text-sm text-muted">Your verified focus versus the remainder of completed study sessions.</p></div>{entries.length ? <div className={`divide-y divide-line-soft ${entries.length > 5 ? "max-h-[20rem] overflow-y-auto" : ""}`}>{entries.map((entry) => <div key={entry.userId} className={`grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-4 px-5 py-4 ${entry.userId === currentUserId ? "bg-moss/5" : ""}`}><span className="truncate font-semibold">#{entry.rank} {entry.displayName}{entry.userId === currentUserId ? " (You)" : ""}</span><span className="tabular text-right text-sm"><b className="text-moss">{formatCompact(entry.focusedMs)}</b><span className="block text-xs text-muted">focused</span></span><span className="tabular text-right text-sm"><b className="text-clay">{formatCompact(entry.unfocusedMs)}</b><span className="block text-xs text-muted">not focused</span></span></div>)}</div> : <p className="p-8 text-center text-sm text-muted">Your focus details will appear here after you complete a study session.</p>}</div>;
}
