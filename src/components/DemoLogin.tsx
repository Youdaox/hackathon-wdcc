        ...
        <form className="mt-7 space-y-4" onSubmit={submit}>
          ...
          <label className="block text-xs font-semibold text-muted">
            ...
            <div className="relative mt-2">
              <input required minLength={8} type={showPassword ? "text" : "password"} autoComplete={mode === "login" ? "current-password" : "new-password"} value={password} onChange={(event) => setPassword(event.target.value)} className="w-full rounded-xl border border-line bg-surface-2 px-4 py-3 pr-16 text-sm font-semibold text-ink" />
              <button
                type="button"
                aria-label={showPassword ? "Hide password" : "Show password"}
                className="absolute inset-y-0 right-3 flex items-center text-[10px] font-bold uppercase tracking-[0.12em] text-moss"
